...
class Template(BaseModel):
    ...
    def validate_non_empty_template(self) -> Template:
        if not (self.items or self.triggers or self.discovery_rules):
            raise ValueError("Template must have at least one item, trigger, or discovery rule.")
        return self
...
class HostMacro(BaseModel):
    ...
    def macro_format(cls, v: str) -> str:
        if "{$" not in v and "{#" not in v:
            raise ValueError(
                f"Invalid macro '{v}'. Must start with {{$...}} (user macro) or {{#...}} (LLD macro)."
            )
        return v
...
class Host(BaseModel):
    ...
    @classmethod
    def host_name_not_empty(cls, v: str) -> str:
        if not v or not v.strip():
...
# ---------------------------------------------------------------------------
# Agent deployment configuration (optional, used by `zbx agent deploy`)
# ---------------------------------------------------------------------------


class AgentConfig(BaseModel):
    files: dict[str, str] = Field(default_factory=dict)  # {dest: source}
